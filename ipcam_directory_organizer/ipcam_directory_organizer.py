...
def combine_videos_from_sequences(directory, video_sequences):
    ...
    for video_sequence_idx, video_sequence in enumerate(video_sequences):
        ...
        ffmpeg_helper.build_concat_demuxer_list([os.path.join(directory, video_filename) for video_filename in video_sequence], concat_demuxer_filename)
        ...
        subprocess.run(ffmpeg_concat_videos_cmd, shell=True, stdout=subprocess.PIPE)
...
def group_ipcam_videos(directory):
    ...
    all_videos = sorted([filename for filename in os.listdir(directory) if filename.endswith(".mp4")])

    print(f"[DEBUG] '{len(all_videos)}' videos found in directory '{directory}'")
    ...
    for video_filename in all_videos:
        ...
        print(f"[DEBUG] Analyzing element with index: '{current_idx}' and previous index '{previous_idx}'")
        ...
        if initial_conditions:
            ...
            if (previous_idx + 1) == current_idx:
                ...
                print(f"[DEBUG] Finished sequence of length '{len(current_video_sequence)}'")
                current_video_sequence = list()
                previous_idx = current_idx
...
if __name__ == "__main__":
    ...
    argparse_parser.add_argument( "-i", "--input-directory", help="The directory to organize videos in", type=str, required=True)
